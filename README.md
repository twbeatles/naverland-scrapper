...
* **📦 PyInstaller 경량/번들 선택:** 기본 배포는 Chromium 미포함(slim)이며, 필요 시 환경변수로 Chromium 번들을 포함할 수 있습니다.
...
기본 배포 프로필은 `naverland-scrapper.spec` 기준 **onefile(slim, Chromium 미포함)**입니다.
...
* onefile(slim, 기본): `dist/naverland_onefile_slim.exe`
...
* 기본 빌드는 Chromium 번들을 포함하지 않는 slim 프로필이며, 필요 시 `NAVERLAND_BUNDLE_CHROMIUM=1`로 번들 포함 빌드를 생성할 수 있습니다.
