...
* **🔗 article-only 역조회 보강:** `fin.land` 상세 페이지의 현재 `complexNumber` payload를 해석하고, live smoke의 article lookup은 Playwright async loop 밖에서 실행해 browser fallback 경로를 실제로 검증합니다.
