...
* **🚦 Live Smoke CLI 추가:** `python app_entry.py --live-smoke [--smoke-headless] [--smoke-url ...]`로 GUI 없이 Playwright 실제 접근 경로를 점검할 수 있습니다.
...
실사이트 접근 확인은 `python app_entry.py --live-smoke --smoke-headless`로 수행할 수 있으며, 필요 시 `--smoke-url`로 특정 경로를 추가 점검할 수 있습니다.
