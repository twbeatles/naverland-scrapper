...
### **v15.0 Deployment Packaging Verification (2026-06-08)**

* **🔔 frozen 알림 번들 보강:** `plyer`는 시스템 알림용 동적 import라 PyInstaller hidden import에 명시해 frozen `--preflight`의 선택 라이브러리 누락 경고를 제거했습니다.
* **📦 Python 미설치 PC 배포 기준 재확인:** 기본 onedir 산출물은 Python DLL, Playwright Chromium/headless shell, Windows Selenium Manager를 포함합니다.
* **🧪 번들 Chromium 검증 스위치:** `NAVERLAND_SKIP_LOCAL_CHROME=1`로 설치된 Chrome 탐지를 우회해 packaged Playwright Chromium 경로를 직접 확인할 수 있습니다.
* **✅ 배포 검증 기준:** source checks 후 `python -m PyInstaller --clean --noconfirm naverland-scrapper.spec`, frozen `--preflight`, frozen `--live-smoke` 순서로 확인합니다.
...
* bundled Chromium 검증: PowerShell에서 `$env:NAVERLAND_SKIP_LOCAL_CHROME='1'; dist\naverland\naverland.exe --live-smoke --smoke-headless`
...
* 2026-06-08 배포 검증 기준으로 시스템 알림용 `plyer` 동적 import를 hidden import에 추가했습니다. 기본 onedir 산출물은 Python DLL, Playwright Chromium/headless shell, Windows Selenium Manager를 포함해야 하며 frozen `--preflight`에서 선택 라이브러리 누락 경고가 없어야 합니다. `NAVERLAND_SKIP_LOCAL_CHROME=1`은 로컬 Chrome 대신 packaged Chromium 경로를 검증할 때 사용합니다.
