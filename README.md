...
* **🚦 Live Smoke CLI 추가:** `python app_entry.py --live-smoke [--smoke-headless] [--smoke-url ...] [--smoke-complex-id ...] [--smoke-article-id ...] [--smoke-json-log smoke.json]`로 GUI 없이 Playwright 실제 접근, article-only 역조회, Geo marker 전환 경로를 점검할 수 있습니다.
...
실사이트 접근 확인은 `python app_entry.py --live-smoke --smoke-headless`로 수행할 수 있습니다. 기본적으로 `home + complex + detail + article-only 역조회 + geo marker switch/API` probe를 실행하며, 필요 시 `--smoke-url`로 경로를 추가하고 `--smoke-complex-id` / `--smoke-article-id`로 샘플 ID를 덮어쓸 수 있습니다. 결과를 남길 때는 `--smoke-json-log logs/live-smoke.json`을 추가합니다.
...
- Packaging / docs / ignore review:
  ...
  - GitHub CI는 parser/database/ui wiring/runtime smoke 핵심 pytest subset과 정적 검사, preflight 점검을 수행합니다.
- Validation:
