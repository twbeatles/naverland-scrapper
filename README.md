...
기본 배포 프로필은 `naverland-scrapper.spec` 기준 **onefile(Chromium 번들 포함)**입니다.

* 기본(onefile, Chromium 번들 포함): `pyinstaller naverland-scrapper.spec`
...
* slim(onefile): PowerShell에서 `$env:NAVERLAND_BUNDLE_CHROMIUM='0'; pyinstaller naverland-scrapper.spec`
...
* onefile(Chromium 번들 포함, 기본): `dist/naverland_onefile.exe`
...
* 기본 빌드는 Chromium 번들을 포함하며, 필요 시 `NAVERLAND_BUNDLE_CHROMIUM=0`으로 slim 빌드를 생성할 수 있습니다.
