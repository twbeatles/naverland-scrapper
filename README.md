...
* **🧠 검색/캐시 동작 개선:** 검색 이력 dedupe 키를 확장하고, 0건 결과도 short TTL로 negative cache 처리합니다.
...
* `naverland-scrapper.spec` 점검 결과, v15.0 변경(재시도 취소/URL 배치 비동기/negative cache/DB 정합성/DB lock·손상 대응/버전 통합) 기준에서도 기존 PyInstaller hidden import 구성으로 빌드 가능하여 추가 수정이 필요하지 않습니다.
