...
**조치 상태**: 1~2단계 핵심 항목 반영 완료 — Article API 페이지네이션(사전 필터 `list_count`, 부분 페이지 보존), response-capture fallback 보충 페이지(`_supplement_article_api_pages`), `configure_paths`/`reset_configured_paths`, `SettingsManager.reset_for_tests`, live-smoke `build_article_api_url` 통합. pytest **305 passed**.
...
### 3단계 (구조 및 TDD 개선) — **부분 완료**

- [x] `article_api` URL 빌더를 live-smoke `probes.py`와 통합 + page-2 smoke probe
- [ ] Geo sweep 통합 테스트 (marker handler + dedup) — 추후
- [x] Claude.md pytest 카운트 동기화 (`305 passed`)
