...
class PlaywrightCrawlerEngine(CrawlerEngine):
    ...
    async def _run_geo(self):
        ...
        marker_handler = self._build_marker_handler(discovered)
        self._desktop_page.on("response", marker_handler)
        try:
            for asset_type in geo.asset_types:
                if self.thread._should_stop():
                    break
                await self._scan_geo_asset_type(asset_type, lat, lon, zoom, geo)
        finally:
            try:
        ...
        if not ordered:
            self.thread.log("ℹ️ 지도 탐색 결과 단지를 찾지 못했습니다.", 30)
            return
        ...
        for row in ordered:
            ...
            for trade_type in self.thread.trade_types:
                if self.thread._should_stop():
                    break
                current += 1
                processed_pairs.add((cid, trade_type))
                self.thread.progress_signal.emit(
                    int(current / total * 100) if total else 0,
                    f"{name} ({trade_type})",
                    self.thread._estimate_remaining_seconds(current, total),
    ...
    def _build_marker_handler(self, discovered: dict[str, dict]):
        async def _consume(response):
            ...
            for raw_marker in payload:
                ...
                if not cid:
                    continue
                current = discovered.get(cid)
                if current is None or int(marker.get("count", 0)) >= int(current.get("count", 0)):
                    discovered[cid] = marker
                    self.thread.register_discovered_complex(marker)

        def _handle(response):
            try:
                asyncio.create_task(_consume(response))
            except Exception:
                return None

        return _handle

    async def _scan_geo_asset_type(self, asset_type: str, lat: float, lon: float, zoom: int, geo):
        if not self._desktop_page:
            return
        base_kind = "houses" if asset_type == "VL" else "complexes"
        trade_code = _TRADE_TO_CODE.get(self.thread.trade_types[0], "A1") if self.thread.trade_types else "A1"
        url = (
        ...
        try:
            ...
            self.thread.log("ℹ️ 지도 canvas 대기 타임아웃", 10)
        ...
        for idx, (target_lat, target_lon) in enumerate(coords, 1):
            if self.thread._should_stop():
                break
            self.thread.log(f"   ↪ {asset_type} sweep {idx}/{total}", 10)
    ...
    async def _crawl_target_with_cache(
        ...
        *,
        ...
        if cache:
            cached = cache.get(cid, trade_type)
        ...
        if cache:
            if raw_items:
                cache.set(cid, trade_type, raw_items)
            else:
                ttl_seconds = int(max(0, self.thread.negative_cache_ttl_minutes) * 60)
                if ttl_seconds > 0:
                    cache.set(cid, trade_type, [], ttl_seconds=ttl_seconds)
            ...
            async def _consume(response):
                ...
                for article in article_list:
                    ...
                    item = normalize_article_payload(
                        ...
                        lon=source_lon,
                        zoom=source_zoom,
                        marker_id=marker_id or cid,
                    )
                    aid = str(item.get("매물ID", "") or "")
    ...
    async def _enrich_items_with_mobile_details(self, items: list[dict]) -> list[dict]:
        ...
        async def _fetch_one(item: dict) -> dict:
            page = await self._page_pool.get()
            try:
                detail = await fetch_mobile_article_detail(page, str(item.get("매물ID", "")))
            except Exception:
                detail = {}
            finally:
                await self._page_pool.put(page)
    ...
    async def _switch_to_listing_markers(self):
        ...
        for text in ["상세매물검색", "매물", "매물검색", "매물 보기"]:
            try:
                ...
                continue
        try:
            await self._desktop_page.locator('button:has-text("단지")').first.click(timeout=1200)
            await self._desktop_page.wait_for_timeout(300)
