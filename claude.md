...
`scripts/ci_check.ps1`는 GitHub Actions `ci.yml`과 동일한 compileall → pyright → pytest subset → preflight 순서를 로컬에서 재현합니다. 푸시 전에 실행하면 CI Pyright 실패를 사전에 잡을 수 있습니다.

최근 확인:

- pytest 전체 통과: `314 passed`
- perf baseline 통과
