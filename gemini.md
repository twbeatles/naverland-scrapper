...
- **Key Features**: 
    ...
    - `complex` 모드 APT-only 정책(VL은 `geo_sweep` 경로만 허용)
...
## 0-8. v15.0.8 Functional Audit Execution (2026-03-10)
- fallback 경계 정합:
  - Playwright 성공 pair를 thread 범위로 누적하고, fallback 시 현재 단지 부분 성공(`count`, `trade_types`) + processed pair를 Selenium prefill로 전달.
  - fallback 지표(`fallback_trigger_count`, `fallback_last_reason`)를 `stats_signal` payload에 반영.
- Selenium negative cache 안정화:
  - `confirmed_empty` 판정이 참일 때만 0건 negative cache 저장.
- 설정값 `0` 보존:
  - `max_retry_count`, `geo_grid_rings` 로딩 경로에서 `or default` 패턴 제거.
  - `retry_on_error=False`일 때 Geo `max_retry_count=0` 전달 보장.
- 스냅샷 자산 분리:
  - `price_snapshots.asset_type` 컬럼/인덱스 마이그레이션 및 legacy `APT` backfill.
  - 스냅샷 저장/조회/API/통계 단지 소스를 자산 기준으로 분리.
  - `add_price_snapshots_bulk`는 7-tuple(legacy) + 8-tuple(asset 포함) 모두 수용.
- `complex` 모드 정책:
  - DB/그룹/예약 로딩에서 VL 제외 및 안내 로그 출력.
- Geo 이력 노이즈 제거:
  - 단지별 trade 성공이 없으면 이력/완료 시그널 생략.
- 차단 쿨다운/관측성:
  - block-like 신호 연속 3회 감지 시 60초 쿨다운.
  - 신규 지표 `block_detect_count`, `block_cooldown_count`, `response_seen_count`, `detail_fetch_total`, `detail_fetch_success` 추가.
- 검증:
  - `python -m unittest discover -s tests -p "test_*.py"` -> `Ran 120 tests` pass.
- `.spec`/`.gitignore`:
  - `naverland-scrapper.spec` 추가 수정 불필요.
  - `.gitignore` 현 규칙으로 충분(변경 없음).
